...
export function* getSnapshotFromUserAuth(userAuth,additionalData){
  try{
 const userRef= yield call(createUserProfileDocument,userAuth,additionalData)/* const userRef = await createUserProfileDocument(userAuth) */
...
export function* signOut(){
  try{
    yield auth.signOut()
    yield put(signOutSuccess()) /* currentUser:null */
  }catch(e){
...
export function* signUp({payload:{email,password,displayName}}){
   try{
      ...
      yield put(signUpSuccess({user,additionalData: {displayName}}))
        ...
        yield put(signUpFailure(error)) /* dispatch */
    }
}
...
export function* onGoogleSignInStart(){
  yield takeLatest(UserActionTypes.GOOGLE_SIGN_IN_START,signInWithGoogle) /* takeLatest listener */
}
...
]) /* make sure we're listening for that action */
}